...
public:
    ...
    void SetScore(unsigned score) noexcept { score_ = score; }
...
public:
    ...
    // Recreates a dog with a previously-assigned id and full state, used when
    // restoring a saved game. Advances the session's id counter so future
    // AddDog calls don't collide with the restored id.
    Dog* RestoreDog(Dog::Id id, const std::string& name, Point2D position, Speed2D speed,
                     Direction direction, unsigned score, const Dog::Bag& bag, size_t bag_capacity);

    // Re-inserts a lost object with a previously-assigned id, advancing the
    // session's id counter accordingly.
    void RestoreLostObject(uint32_t id, const LostObject& object);
...
public:
    ...
    // Recreates a player with a previously-assigned id, used when restoring a
    // saved game. Advances the static id counter so future players don't
    // collide with the restored id.
    Player(PlayerId id, Dog* dog, GameSession* session);
...
public:
    ...
    // Registers a previously-generated token for a restored player without
    // generating a new one.
    void RestoreToken(const Token& token, Player& player);
...
public:
    ...
    // Recreates a player with a previously-assigned id, used when restoring a
    // saved game.
    Player& Restore(PlayerId id, Dog* dog, GameSession* session);
...
public:
    ...
    const GameSession* FindSession(const Map::Id& id) const noexcept {
        if (const auto it = map_id_to_session_index_.find(id); it != map_id_to_session_index_.end()) {
            return sessions_.at(it->second).get();
        }
        return nullptr;
    ...
    // Returns the session for the given map, creating it if it doesn't exist
    // yet. Returns nullptr if the map itself is unknown.
    ...
    // Recreates a player bound to an existing dog/session with a
    // previously-generated token, used when restoring a saved game.
    Player& RestorePlayer(PlayerId id, Dog* dog, GameSession* session, const Token& token);
