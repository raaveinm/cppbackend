...
Dog* GameSession::RestoreDog(Dog::Id id, const std::string& name, Point2D position, Speed2D speed,
                              Direction direction, unsigned score, const Dog::Bag& bag, size_t bag_capacity) {
    auto dog = std::make_unique<Dog>(id, name, position, this, bag_capacity);
    dog->SetSpeed(speed);
    dog->SetDirection(direction);
    dog->SetScore(score);
    for (const auto& item : bag) {
        if (!dog->CollectItem(item)) {
            throw std::runtime_error("Failed to restore dog's bag contents");
        }
    }
    dogs_.emplace_back(std::move(dog));
    dog_id_ = std::max(dog_id_, *id + 1);
    return dogs_.back().get();
}

void GameSession::RestoreLostObject(uint32_t id, const LostObject& object) {
    lost_objects_.emplace(id, object);
    lost_object_id_ = std::max(lost_object_id_, id + 1);
}
...
std::pair<Player&, Token> Game::AddPlayer(const Map::Id& map_id, const std::string& player_name) {
    auto* session = GetOrCreateSession(map_id);
    if (!session) {
...
Player& Game::RestorePlayer(PlayerId id, Dog* dog, GameSession* session, const Token& token) {
    auto& player = players_.Restore(id, dog, session);
...
Player::Player(PlayerId id, Dog* dog, GameSession* session)
    : id_{id}, dog_{dog}, session_{session} {
    player_id_s = std::max(player_id_s, *id + 1);
}
...
const Token& Player::GetToken() const noexcept {
    return *token_;
}
...
void PlayerTokens::RestoreToken(const Token& token, Player& player) {
    token_to_player_.emplace(token, &player);
}
...
Player& Players::Restore(PlayerId id, Dog* dog, GameSession* session) {
    players_.emplace_back(std::make_unique<Player>(id, dog, session));
    return *players_.back();
}
