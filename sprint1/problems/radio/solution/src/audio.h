...
class Recorder {
    ...
    void SaveBuffer(
        ...
        size_t size = std::min(static_cast<size_t>(frameCount * frame_size_),buffer_.size() - current_off_);
...
class Player {
    ...
    void FillBuffer(void* pOutput, const ma_uint32 frameCount) {
        size_t size = std::min(static_cast<size_t>(frameCount * frame_size_),
                               max_frame_ * frame_size_ - current_off_);

        std::copy_n(current_buffer_ + current_off_, size, static_cast<char*>(pOutput));
...
public:
    Player(ma_format format, int channels) {
        ...
        frame_size_ = ma_get_bytes_per_frame(format, channels);
        init_result_ = ma_device_init(nullptr, &device_config, &device_);
    }
