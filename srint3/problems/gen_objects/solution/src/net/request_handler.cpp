...
http::response<http::string_body> RequestHandler::MakeMapsListResponse(unsigned version, bool keep_alive) const {
...
http::response<http::string_body> RequestHandler::MakeMapDescriptionResponse(const model::Map& map, unsigned version, bool keep_alive) {
    http::response<http::string_body> response(http::status::ok, version);
    response.set(http::field::content_type, "application/json");
    response.set(http::field::cache_control, "no-cache");
    response.keep_alive(keep_alive);
